...
use crate::{Name, Player, Position, Renderable, State, Item, Impassable, RequiresItem, Door, PermanentItem, SerializeMe};
...
pub fn build_door(gs: &mut State, name: String, coord: (i32, i32), image: char, key: Entity) -> Entity {
    gs.ecs
        .create_entity()
        .with(Name { name })
        .with(Position { x: coord.0, y: coord.1 })
